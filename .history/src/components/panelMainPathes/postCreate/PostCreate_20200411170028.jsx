...
class PostCreate extends Component {
  ...
  autoCompleteChangeHandler = (event, value) => {
    ...
    for (let i = 0; i < newValue.length; i++) {
      ...
      let tagName = this.state.topTags.find(tag => newValue[i].includes(tag.tagName)).tagName
      let tagId = this.state.topTags.find(tag => newValue[i].includes(tag.tagName)).tagGuid

      let finalValues = newValue[i].map(val => val === tagName ? val = tagId : val = val);
