...
class PostCreate extends Component {
  ...
  autoCompleteChangeHandler = (event, value) => {
    ...
    // for (let i = 0; i < newValue.length; i++) {
      ...
      let tagName = this.state.topTags.filter(tag => newValue.includes(tag.tagName)).tagName
      let tagId = this.state.topTags.filter(tag => newValue.includes(tag.tagName)).tagGuid

      let finalValues = newValue.map(val => val === tagName ? tagId : val);
      
      console.log(finalValues)

      const optionsDict = {...this.state.topTags.filter(tag => newValue.includes(tag.tagName))}

      let item2 = optionsDict.tagGuid
