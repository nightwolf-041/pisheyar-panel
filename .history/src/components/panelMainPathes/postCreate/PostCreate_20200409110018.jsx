...
import {FormControlLabel, Switch, Button, CircularProgress, TextField, Chip } from '@material-ui/core'
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
...
class PostCreate extends Component {
  ...
  componentDidMount() {
    axiosConfig.get('/Tag/GetAll').then(res => {
      this.setState({topTags: res.data.})
    })
  }
  ...
  if(window.innerWidth < 500){
    ...
    return (
      <PanelMain header={<PanelMainPostCreatehead />}>
        ...
        multiple
        id="tags-filled"
        options={this.state.topTags.map((option) => option.title)}
        defaultValue={[topTags[1].title]}
        freeSolo
