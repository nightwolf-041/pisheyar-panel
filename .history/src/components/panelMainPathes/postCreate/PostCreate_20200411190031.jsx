...
// this.setState({replacedValue: finValue})
 
// }

  autoCompleteChangeHandler = (event, values) => {

    const newValues = values.map(str => str.replace(/^\s+|\s+$|\s+(?=\s)/g, "")).reduce(function(a,b){if(a.indexOf(b)<0)a.push(b);return a;},[])

    this.setState({trimedValues: newValues} , ()=> { 

    let replacedTrimedValues = [...this.state.trimedValues]

    for (var obj in this.state.topTags) {

      let currentName = this.state.topTags[v].tagName
      let currentId = this.state.topTags[v].tagGuid

      for (var a in replacedTrimedValues) {
        if(replacedTrimedValues[a] === currentName){
          replacedTrimedValues[a] = currentId
        }else{
          replacedTrimedValues[a] = replacedTrimedValues[a]
        }
      }
    }
...
// }
  ...
  console.log(this.state.replacedValue);
