...
class PostCreate extends Component {
  ...
  autoCompleteChangeHandler = (event, value) => {
    ...
    let tagName = this.state.topTags.find(tag => value.includes(tag.tagName)).tagName
    let tagId = this.state.topTags.find(tag => value.includes(tag.tagName)).tagGuid

    let finalValues = value.map(val => val === tagName ? tagId : val);

    const newValue = finalValues.map(str => str.replace(/^\s+|\s+$|\s+(?=\s)/g, "")).reduce(function(a,b){if(a.indexOf(b)<0)a.push(b);return a;},[])
