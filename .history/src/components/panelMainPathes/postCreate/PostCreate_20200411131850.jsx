    ...
    return (
      ...
      <Autocomplete
        ...
        onChange={(event, value) => {
          const newValue = value.map(string => string.replace(/\s/g, ""))
          console.log(newValue)
          console.log(newValue.reduce(function(a,b){if(a.indexOf(b)<0)a.push(b);return a;},[]))
        }}
        renderTags={(value, getTagProps) => {
          console.log(value)
          let newVal = value.map(string => string.replace(/\s/g, ""))
          console.log(newVal)
          let finalVal = newVal.reduce(function(a,b){if(a.indexOf(b)<0)a.push(b);return a;},[])
          finalVal.map((option, index) => (
            <Chip 
            // variant="filled"
            // variant="filled"
            label={option}
