...
// import '../../../../node_modules/react-filepond/';
...
class PostCreate extends Component {
  ...
  autoCompleteChangeHandler = (event, values) => {
    ...
    for (var obj in this.state.postTopTags) {
      ...
      let currentName = this.state.postTopTags[obj].tagName
      let currentId = this.state.postTopTags[obj].tagGuid
  ...
  sendDataHandler = () => {
    ...
    let postImageName = postImage.name
    let postImageSize = postImage.size
    let postImageType = postImage.type
    let postImageData = [
      postImageName,
      postImageSize,
      postImageType
    ...
    if(postImage.length === 0 || titleValue.length === 0 || abstractValue.length === 0 ||   descriptionValue.length === 0 || postTags.length === 0 || categoryName.length === 0) {
    ...
    return (
      <PanelMain header={<PanelMainPostCreatehead />}>
        ...
        <FilePond ref={ref => this.pond = ref}
          ...
          server = {{
            url: 'http://185.94.97.164/api/uploader/FilepondProcess',
            process: '/UploadImageTest',
            // revert: '/RevertImage',
            revert: {
              url: '/RevertImage',
              method: 'POST'
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            ...
            variant="outlined" label={this.state.loadingPostTags ? 'درحال بارگیری...' : 'تگ های پست' }
