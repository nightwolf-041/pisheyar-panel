...
class PostCreate extends Component {
  ...
  componentDidMount() {
    ...
    axiosConfig.get('/Tag/GetAll', {
      ...
      setTimeout(() => {
        this.setState({
          topTags: res.data.tags,
          loadingAuto: false
        })
      }, 3000);
    }).catch(err => {
      toast('خطا در بارگیری تگ ها', {type: toast.TYPE.ERROR});
      this.setState({
        topTags: [],
        loadingAuto: false
      })
  ...
  autoCompleteChangeHandler = (event, values) => {
    ...
    const newValues = values.map(str => str.replace(/^\s+|\s+$|\s+(?=\s)/g, ""))
    ...
    return (
      <PanelMain header={<PanelMainPostCreatehead />}>
        ...
        <Autocomplete
          multiple
          id="tags-filled"
          loading={this.state.loadingAuto}
          options={this.state.topTags.map((option) => option.tagName)}
          // defaultValue={[topDefTags[1].title]}
          freeSolo
          ...
          renderInput={(params) => (
            ...
            variant="outlined" label={this.state.loadingAuto ? 'درحال بارگیری...' : 'تگ های پست' }
            placeholder="انتخاب یا تایپ کنید"
            InputProps={{
              ...params.InputProps,
              endAdornment: (
                <React.Fragment>
                  {this.state.loadingAuto ? <CircularProgress color="inherit" size={20} /> : null}
