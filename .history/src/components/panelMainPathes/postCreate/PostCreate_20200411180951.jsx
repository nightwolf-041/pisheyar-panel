...
class PostCreate extends Component {
  ...
  autoCompleteChangeHandler = (event, value) => {
    ...
    const newValue = value.map(str => str.replace(/^\s+|\s+$|\s+(?=\s)/g, "")).reduce(function(a,b){if(a.indexOf(b)<0)a.push(b);return a;},[])

    this.setState({trimedValue: newValue})
    console.log(newValue);

    // for (let i = 0; i < newValue.length; i++) {
      
      // let tagName = this.state.topTags.find(tag => newValue.includes(tag.tagName)).tagName
      // let tagId = this.state.topTags.find(tag => newValue.includes(tag.tagName)).tagGuid
    ...
    let finValue = [...this.state.trimedValue]

    for (var v in this.state.topTags) {
      // var data = {...this.state.topTags.find(tag => newValue.includes(tag.tagName))};
      // var name = data.tagName
      let currentName = this.state.topTags[v].tagName
      let currentId = this.state.topTags[v].tagGuid

      // return {currentName, currentId}

      // let final = newValue.map(val => val === currentName ? currentId : val);
      // console.log(final);

      for (var a in finValue) {
        if(finValue[a] === currentName){
          finValue[a] = currentId
        }else{
          finValue[a] = finValue[a]
        }
      }
...
console.log(finValue);
  ...
  // componentDidUpdate() {
  //   let tags = [...this.state.topTags]
  //   let values = [...this.state.trimedValue]
  
  //   let item = {...tags.find(tag => values.includes(tag.tagName))}
  //   let name = item.tagName
  // //   let index = values.indexOf(name);
  // //   if ( values.includes(name)) {
  // //     values[name] = item.tagGuid;
  // // }
  

  // values = values.map(function(val) { return val === name ? item.tagGuid : val; });
    ...
    return (
      ...
      <Autocomplete
        ...
        onChange={(event, value) => {
          this.autoCompleteChangeHandler(event, value)
        }}
        renderTags={(value, getTagProps) =>
          this.state.trimedValue.map((option, index) => (
            <Chip 
            // variant="filled"
            label={option}
            {...getTagProps({ index })} />
